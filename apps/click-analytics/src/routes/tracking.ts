...
import { requireSiteKey } from "../middleware/auth";
...
router.post(
  ...
  async (req, res) => {
    ...
    const queued = toQueuedEvent(req.body, ip, req.websiteId!);
...
router.post(
  ...
  async (req, res) => {
    ...
    const events: QueuedEvent[] = (req.body.events as ClickEventInput[]).map(
      (e) => toQueuedEvent(e, ip, req.websiteId!)
    );
    ...
    res
      .status(202)
      .json({ queued: events.length, timestamp: new Date().toISOString() });
  }
);
