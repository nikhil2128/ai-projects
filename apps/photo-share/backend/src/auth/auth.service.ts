...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    // Kick off async profile verification (non-blocking)
    await this.verificationService.enqueueVerification(saved.id);
  ...
  async login(dto: LoginDto) {
    const user = await this.userRepository.findOne({
      ...
      select: ['id', 'username', 'email', 'password', 'displayName', 'avatarUrl', 'verificationStatus'],
    });
    if (!user) {
