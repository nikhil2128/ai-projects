...
router.post("/", requireRole("writer"), (req: Request, res: Response) => {
...
router.put("/:id", requireRole("writer"), (req: Request, res: Response) => {
	...
	if (values !== undefined) {
		if (!isObjectRecord(values)) {
			res
				.status(400)
				.json({ success: false, error: "values must be an object" });
			return;
		}
...
router.delete("/:id", requireRole("writer"), (req: Request, res: Response) => {
	...
	if (normalized.createdBy && normalized.createdBy !== req.user!.id) {
		res.status(403).json({
			success: false,
