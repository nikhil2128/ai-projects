...
export function createOrderRoutes(orderService: OrderService): Router {
  ...
  router.get("/internal/:id", (req: Request, res: Response) => {
    const result = orderService.getOrderInternal(req.params.id);
    if (!result.success) {
  ...
  router.put("/internal/:id/status", (req: Request, res: Response) => {
  ...
  router.get("/:id", (req: Request, res: Response) => {
    ...
    const result = orderService.getOrder(req.params.id, userId);
    if (!result.success) {
      res.status(404).json({ error: result.error });
      return;
    }
