...
import { HttpProductClient, HttpCartClient, HttpOrderClient } from "../shared/http-clients";

function startServer(app: Express.Application): Promise<{ server: http.Server; port: number }> {
  return new Promise((resolve) => {
    const server = (app as ReturnType<typeof http.createServer>).listen(0, () => {
...
describe("E-commerce Microservices Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const productClient = new HttpProductClient(`http://localhost:${product.port}`);
  ...
  describe("Product Routes", () => {
    ...
    it("GET /api/products should list products", async () => {
      ...
      const res = await request(gatewayApp).get("/api/products?category=ListTest");
      expect(res.status).toBe(200);
  ...
  describe("Full E-commerce Flow: Register -> Search -> Cart -> Order -> Payment", () => {
    ...
    it("should handle order cancellation and refund flow", async () => {
      ...
      const products = await request(gatewayApp).get("/api/products?category=FlowTest");
      const mouseId = products.body.find(
        (p: { name: string }) => p.name === "Wireless Mouse"
      ).id;

      const mouseBefore = await request(gatewayApp).get(`/api/products/${mouseId}`);
    ...
    it("should handle cart manipulation (update quantity, remove items)", async () => {
      ...
      const clothing = await request(gatewayApp).get("/api/products?category=FlowClothing");
